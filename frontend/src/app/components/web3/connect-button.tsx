  ...
  if (!activeAccount)
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent className="min-w-[14rem] bg-gray-500">
          {!activeAccount &&
            browserWallets.map((w) =>
              isWalletInstalled(w) ? (
                <DropdownMenuItem
                  key={w.id}
                  className="cursor-pointer text-white hover:bg-gray-700 px-4 py-2"
                  onClick={() => {
                    connect?.(undefined, w)
                  }}
                >
  ...
  return (
    <div className="flex select-none flex-wrap items-stretch justify-center gap-4">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          asChild
          className="rounded-2xl bg-gray-400 px-4 py-6 font-bold text-foreground"
        >
      <button className="min-w-[14rem] border translate-none flex items-center justify-between gap-2 px-4 py-3 font-bold text-foreground">
        <div className="flex flex-col items-center justify-center">
          {/* Assuming AccountName is a component that renders the account's name. 
              You might need to adjust this part to directly display the account name 
              if you're not using a separate component. */}
